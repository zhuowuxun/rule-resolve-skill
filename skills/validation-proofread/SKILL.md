...
2. Confirm the target project, owner API base, and dictionaries before changing anything.
   Bind the proofreading run to the platform that owns the project. If the project is on `http://192.168.10.89`, use the 10.89 backend/database for dictionary inspection, DB backup, `tools/validation/check_and_fix.py`, manual SQL fixes, and export. Do not start or use a local backend/DB for that remote project ID.
