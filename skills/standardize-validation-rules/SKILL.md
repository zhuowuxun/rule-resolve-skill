...
3. Apply validation-only naming rules.
   ...
   - Product-level AI classification must be stable across different commands, endpoints, or vulnerable paths for the same software. For example, all `9Router` and `Flowise` vulnerability rows should use `AI应用程序漏洞`.
   ...
   - Use `AI应用程序漏洞` for AI application products such as 9Router, Flowise, Langflow, Open WebUI, OpenClaw, MLflow, LiteLLM, MindsDB, LibreChat, NocoBase, opencode, and short-video-maker.
   ...
   - For command-and-control rows, keep malware/sample names (e.g., `POLLREGISTER`, `LIGHTPHOENIX`, `AEROSTAT`) in English when the description contains them as a backdoor/family/sample name. Avoid awkward Chinese literal translations such as `轮毂登记器` for `POLLREGISTER` or `轮询器` for the polling component; if a malware appears in multiple rows, keep the same English spelling across all rows so the variant set is easy to recognize.
   ...
   - Normalize Chinese malware-type wording `远程控制木马` to `远控木马` in rule titles and descriptions.
...
9. Run delivery QA and manual sampling before final response.
   ...
   - Raw vulnerability names with trailing `Variant-n` / `变种 #n` must still be classified and prefixed. Extract the variant first, standardize the vulnerability title, then append `变种 #n`; do not leave rows such as `Windows Defender RoguePlanet 权限提升漏洞 变种 #2` without a validation prefix.
