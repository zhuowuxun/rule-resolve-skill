...
3. Apply validation-only naming rules.
   ...
   - For AD-domain-related network rows that are judged to contain a PCAP package or captured traffic evidence, add `AD侦查流量` in the title after the prefix, for example `命令与控制 - AD侦查流量，...`; do not use the generic `AD域` title segment for these network-traffic rows.
