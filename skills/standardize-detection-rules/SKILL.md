...
3. Standardize `desc`.
   ...
   - if the vulnerability phrase contains an extra technique detail such as `Ghost Bits 截断`, `Ghost Bits 复合`, or `Ghost Bits 宽松归一化`, treat it as a bypass method, not as another vulnerability name; use `核心漏洞的 Ghost Bits XXX 绕过，变种 #n 的利用尝试`
